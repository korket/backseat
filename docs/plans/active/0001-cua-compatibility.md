...
Open items: production wrapper work proceeds under `docs/plans/active/0005-run-persistence.md`; the core contracts, Cua CLI adapter, and session lifecycle are completed in `docs/plans/completed/`. The first milestone is demonstrated end to end in `experiments/cua-driver/milestone-background-action.md`. `docs/decisions/0004-foreground-input-policy.md` is accepted.
